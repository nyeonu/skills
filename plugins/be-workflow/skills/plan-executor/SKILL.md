...
- 서브에이전트에게 계획 문서 전체를 넘기지 마라. 필요한 것만 추려서 전달한다: 목표 요약, 해당 작업 섹션 전문, 범위의 "하지 말 것". 컨텍스트가 커질수록 저비용 모델의 정확도가 떨어지고 토큰 비용이 늘어난다.
...
- Test names (`@DisplayName` or the test function name) state what is
  ...
  written in the task. Never prefix them with an id or a number; the plan's
...
- 모든 작업이 `done`이면 계획 문서의 "전체 검증" 절차를 직접 실행한다.
- 통과하면 계획 `status: done`으로 갱신하고 아래 5번 형식으로 사용자에게 보고한다.
...
테스트 대조는 새 판단을 만드는 단계가 아니다 — **계획의 추적성 표(기준 → 테스트 → 작업)를 기준선으로, 실제 테스트 코드·실행 결과를 기계적으로 대조**한다. 기준선 없는 커버리지 판정(라인 % 등)으로 대체하지 않는다. 재료는 서브에이전트 보고의 `tests`·`uncovered` 슬롯 취합 + 오케스트레이터가 직접 실행한 verify 결과다. 대조 후 실제 테스트 이름이 계획 표와 다르면 **계획 표를 실제 이름으로 갱신한다** — 이 표는 기준 ↔ 테스트의 단일 색인이라 낡으면 spec-conformance-check의 추적성 판정이 무너진다. 테스트 이름은 기준 식별자 없이 무엇을 검증하는지 서술해야 하므로(tdd 스킬), 서브에이전트가 식별자나 번호 접두어를 붙였거나 이름이 무엇을 검증하는지 불명확하면 대조 표의 결과 열에 결함으로 적는다.

```markdown
## 실행 결과

- 계획: <계획 문서 경로>
- 완료한 작업: <완료 T-N 목록 / 전체 개수. 재시도나 tier 상향이 있었으면 함께 적는다>
- 바뀐 파일: <경로 목록>
- 판정: <전체 검증 통과 여부>
...
|---|---|---|---|
| birthYear가 1900 미만이면 400으로 거절한다 (②/유닛) | 잘못된 생년월일은 거절한다 | AdditionalInfoServiceTest · "birthYear가 1900 미만이면 400으로 거절한다" | PASS |
| PUT 실패 시 upsert가 롤백된다 (③/통합) | 저장 실패 시 이전 값이 유지된다 | 미작성 | — |
...
- 실행하지 못한 검증: <없으면 "없음">
- 미커버 분기: <서브에이전트 보고 취합. 계획에 없던 오류 분기·엣지 케이스. 없으면 "없음">
- 다시 판단할 조건: <이 판정이 무효가 되는 전제 변화. 없으면 "없음">

## 다음 단계

스펙 적합성 검증(spec-conformance-check)을 진행한다.
```

- "다음 단계"는 생략하지 않는다. 여기서의 검증은 "계획대로 됐는가"까지이며, "스펙에 맞는가"는 별도 단계다.
- 테스트 대조 표에 **미작성이 있으면 완료가 아니다** — 해당 작업의 완료 기준 미충족이므로 3번 검증의 재시도 절차로 되돌린다.
...
| "verify가 다 통과했으니 테스트 대조는 생략하자" | "테스트가 돌았다"와 "계획한 테스트가 전부 존재하고 통과했다"는 다른 명제다. 대조표 없는 완료 보고는 누락을 숨긴다. |
| "서브에이전트 보고가 영어니 사용자 보고에 그대로 붙이자" | 채널 언어와 산출물 언어는 다르다. 사용자 보고·테스트 대조·문서는 한국어 산출물이다 — 영어는 일회성 통신에만 쓴다. |
...
| "전체 검증은 개별 verify가 다 통과했으니 생략하자" | 개별 통과가 통합 동작을 보장하지 않는다. 전체 검증은 별개 단계다. |
| "전체 검증까지 통과했으니 스펙도 충족했을 것이다" | 전체 검증은 계획 기준이다. 계획이 스펙을 잘못 번역했을 가능성은 spec-conformance-check가 잡는다. |
