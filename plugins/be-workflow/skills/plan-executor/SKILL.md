...
구현 작업은 TDD 순서를 따른다: 작업에 명시된 테스트를 먼저 작성하고 실행해
실패를 확인한 뒤(RED), 통과할 최소한을 구현하고(GREEN), 테스트가 통과하는
상태에서 정리하라(REFACTOR). 명시된 테스트 외의 테스트를 임의로 추가하지 마라.
구현 중 계획에 없던 오류 분기·엣지 케이스가 생기면 테스트를 임의로 추가하지
말고 완료 보고의 "미커버 분기" 항목으로 알려라.
테스트를 skip·비활성화하거나 단언을 완화해 통과시키는 것은 실패다 —
통과시킬 수 없으면 그 사실을 보고하라.
...
작업 완료 후: 수정한 파일 목록, 검증 명령 실행 결과, 작성한 테스트
(파일·테스트 이름), 미커버 분기(없으면 "없음")를 보고하라.
```
...
- 작업의 verify 명령을 오케스트레이터가 직접 실행해 확인한다. 테스트 기반 verify는 통과 여부만이 아니라 **skip·비활성화된 테스트가 없는지, 계획에 명시된 테스트가 실제로 존재하는지**도 확인한다 — 테스트를 지우거나 약화시켜 만든 그린은 실패다.
...
완료 보고에 포함한다. 새 판단을 만드는 단계가 아니다 — **계획의 추적성 매핑(SC → 테스트 케이스 → 작업)을 기준선으로, 실제 테스트 코드·실행 결과를 기계적으로 대조**한다. 기준선 없는 커버리지 판정(라인 % 등)으로 대체하지 않는다.
