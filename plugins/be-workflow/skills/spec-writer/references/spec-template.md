...
frontmatter와 목차 순서, 성공 기준의 `§글자-번호` 식별자 형식은 고정이다 — 기계 계약이므로 필드 이름과 값 형식을 바꾸지 않는다. 본문 문장과 분량은 작업 크기에 맞게 조절한다. 필수 항목에 쓸 내용이 없으면 일반론으로 채우지 말고 `없음`이라고 쓴다.
...
<완료를 판별하는 구체적·검증 가능한 조건. 기준은 기능 영역 하나씩 **묶음(§글자)**으로 나누고,
각 항목은 `§글자-번호`(예: `§A-1`, `§A-2`, `§B-1`)로 식별한다. 기준이 적어도 묶음은 최소
하나(§A)다. 이 식별자가 task-breakdown 추적성 표의 키이자 spec-conformance-check의 검증
단위다. 각 기준은 테스트로 옮길 수 있는 형태로 쓴다 (tdd 스킬 출처 ①).

식별자 규칙 — 문서 안에서 유일하고, 한 번 부여하면 바꾸지 않는다. 전역 순번(SC-1, SC-2…)으로
매기지 않는다. 전역 순번은 스펙을 합본할 때 번호가 밀려 테스트·리포트·ADR의 참조가 한꺼번에
어긋나므로 폐기했다:
- 묶음 글자는 A부터 순서대로 부여한다. 묶음이 없어져도 글자를 재사용하지 않는다.
- 묶음 안 번호는 추가 전용이다. 새 기준은 묶음 끝에 붙이고, 없어진 기준은 줄을 지우지 않고
  `§A-2: (삭제 YYYY-MM-DD — 사유)`로 남긴다. 그래야 뒤 번호가 밀리지 않는다.
- 합본(여러 스펙을 통합 스펙 하나로 합치기)이 예정된 증분 스펙은 처음부터 통합 스펙에서 아직
  쓰지 않은 글자를 쓴다(통합 스펙이 §A~§F를 쓰고 있으면 증분 스펙은 §G부터). 그러면 합본 때
  글자와 번호가 그대로 옮겨진다. 합본 시 글자가 겹치면 들어오는 묶음에 미사용 글자를 새로
  부여하고, 옛 스펙(`superseded`) 첫 줄의 `대체: <새 spec_id>` 뒤에 `(§A → §G)`처럼 대응을
  남긴다. 기존 묶음의 글자와 번호는 건드리지 않는다.
- 다른 스펙의 기준은 `<spec_id>#§A-1`, 묶음 전체는 `<spec_id>#§A`로 가리킨다. 같은 문서
  안에서는 `§A-1`, `§A`다.
- 기준 식별자를 테스트 이름(`@DisplayName`, 테스트 함수 이름)에 넣지 않는다. 테스트 이름은
  무엇을 검증하는지 서술하고(tdd 스킬), 기준 ↔ 테스트 연결은 계획(PLAN.md)의 추적성 표가
  담당한다.
...
### §A. <묶음 이름 — 기능 영역 하나. 예: 부가 정보 조회/저장>

- §A-1: <주어진 상태 또는 입력 → 기대 결과>
- §A-2: <주어진 상태 또는 입력 → 기대 결과>

### §B. <묶음 이름>

- §B-1: <주어진 상태 또는 입력 → 기대 결과>

<scope: shared-app 전용>
| 통합 기준 | 검증 담당 | 검증 시점 |
|---|---|---|
| §A-1 | <담당자> | <예: 두 애플리케이션 스테이징 배포 후> |
...
- [ ] 성공 기준이 묶음(`### §A.`) 아래 `§글자-번호`로 식별되고 문서 안에서 유일한가. 전역 순번(SC-N)을 쓰지 않았는가
- [ ] 다른 스펙의 기준은 `<spec_id>#§글자-번호`로 가리켰는가
- [ ] 승인된 스펙의 기준을 고쳤다면 번호를 밀지 않았는가 (추가는 묶음 끝에, 삭제는 `(삭제 …)` 표시, 글자 재사용 없음)
