...
frontmatter와 목차 순서는 고정이다 — 기계 계약이므로 필드 이름과 값 형식을 바꾸지 않는다. 본문 문장과 분량은 작업 크기에 맞게 조절한다. 필수 항목에 쓸 내용이 없으면 일반론으로 채우지 말고 `없음`이라고 쓴다.

이 파일은 **산출물의 모양**만 정한다. scope 결정, 저장 위치와 브랜치, 승인 조건, 승인 후 수정 규칙, status 전이는 spec-writer SKILL.md의 절차를 따른다. 공통 스펙을 어느 저장소에 두는지 같은 팀 운영 규칙은 조직의 운영 문서가 정하며, 에이전트는 읽지 않아도 된다.
...
| `shared-app` | 여러 애플리케이션이 함께 알아야 하는 것만: 계약, 배포 순서, 통합 성공 기준 | 없음 | 필수 | 계약 · 호출 흐름 · 배포 순서 · 계약 변경 절차 |

용어: **애플리케이션**은 따로 배포되고 담당자가 있는 단위(예: auth-api). **저장소**는 git 저장소로, 문서 위치와 브랜치를 말할 때만 쓴다. 링크는 자식 → 부모 한 방향이다. 공통 스펙은 자식 목록을 갖지 않고 `participants`만 갖는다.

## 템플릿

````markdown
---
spec_id: <YYYY-MM-DD-브랜치명-주제-kebab-case>   # shared-app 은 브랜치명 자리에 에픽 티켓 번호. 예: 2026-09-15-TICKET-123-membership-api
title: <스펙 제목>
status: draft                # draft | approved | implemented | superseded. superseded 면 본문 첫 줄에 "대체: <새 spec_id>"
created: <YYYY-MM-DD>
scope: app                   # app | shared-app. 생략하면 app
parent: <공통 스펙 spec_id> @ <저장소>@<통합 브랜치>:<경로>   # 공통 스펙의 일부인 app 스펙만. 예: 2026-09-15-TICKET-123-membership-api @ auth-api@feature/TICKET-123:docs/spec/2026-09-15-TICKET-123-membership-api.md. 해당 없으면 이 줄을 지운다
...
|---|---|---|---|
| <예: GET /v2/membership/extra> | <auth-api> | <service-api> | <요청 파라미터, 응답 형식, 에러 코드> |
| <예: 기존 GET /v1/... 처리> | <auth-api> | <service-api> | <유지 기한 또는 삭제 시점> |
...
예: "auth-api 먼저. 전환 완료까지 기존 API 유지. service-api 배포 후 기존 API 제거는 별도 작업.">
...
<완료를 판별하는 구체적·검증 가능한 조건. 기준은 기능 영역 하나씩 **묶음**으로 나누고,
각 기준은 그 아래 한 문장으로 쓴다. **식별자를 붙이지 않는다 — 기준 문장 자체가 키다.**
이 문장이 task-breakdown 추적성 표의 키이자 spec-conformance-check의 검증 단위다. 각 기준은
테스트로 옮길 수 있는 형태로 쓴다 (tdd 스킬 출처 ①).

식별자를 쓰지 않는 이유 — 번호는 손으로 지켜야 유지되고, 어긋나도 유효해 보인다. 전역
순번(SC-1, SC-2…)은 합본할 때 번호가 밀려 테스트·리포트·ADR 참조가 한꺼번에 어긋났다.
묶음별 번호(`§A-1` 등)도 삭제할 때 줄을 남기고 합본할 때 미사용 글자를 확인해야 유지되는데,
그 규율은 기계가 강제하지 않는다. 문장을 키로 쓰면 규율이 없어지고, 어긋나면 눈에 띈다:
- 기준을 추가하면 묶음 끝에 문장을 붙이고, 없어지면 줄을 지운다. 표시를 남기지 않는다.
- 합본(여러 스펙을 통합 스펙 하나로 합치기)은 묶음별로 목록을 합친다. 같은 묶음 이름이
  양쪽에 있으면 하나로 합치고 기준 문장은 그대로 옮긴다. 조정할 번호가 없다.
- 기준 문장을 고치면 계획의 추적성 표와 검증 리포트의 해당 행도 같이 고친다. 승인된 스펙의
  기준을 고치는 것은 계약 변경이므로 원래 절차를 탄다.
- 다른 스펙은 묶음 단위로 `<spec_id>#<묶음 제목>`(마크다운 앵커)으로 가리키고, 기준 하나를
  특정해야 하면 그 문장을 인용한다.
- 기준 문장을 서로 구별할 수 없을 만큼 비슷하게 쓰지 않는다. 구별이 안 되면 그 둘은
  중복이거나 모호한 기준이다.
- 테스트 이름(`@DisplayName`, 테스트 함수 이름)도 같은 원칙이다. 무엇을 검증하는지 서술하고
  (tdd 스킬), 기준 ↔ 테스트 연결은 계획(PLAN.md)의 추적성 표가 담당한다.

scope: shared-app 이면 통합 성공 기준만 적는다 — 애플리케이션 둘 이상이 함께 배포된 뒤에야
확인되는 조건. 애플리케이션 하나 안에서 확인되는 조건은 그 애플리케이션 스펙의 기준이다.
통합 성공 기준은 검증 담당을 반드시 적는다.>

### <묶음 이름 — 기능 영역 하나. 예: 부가 정보 조회/저장>
...
| 통합 기준 | 검증 담당 | 검증 시점 |
|---|---|---|
| <기준 문장> | <담당자> | <예: 두 애플리케이션 스테이징 배포 후> |
...
- [ ] `shared-app`의 성공 기준이 통합 조건만 담고 있고 각 기준에 검증 담당이 있는가
- [ ] 성공 기준이 묶음(`### <묶음 이름>`) 아래 문장으로 적혔고 식별자(SC-N, `§A-1` 등)를 붙이지 않았는가
- [ ] 기준 문장끼리 서로 구별되는가 (거의 같은 문장 둘이 없는가)
- [ ] 다른 스펙은 묶음 앵커로 가리키거나 기준 문장을 인용했는가
